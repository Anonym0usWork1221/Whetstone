...
#define HGROUP 32 /* weights per (ls, lm) pair -- one uint4 exactly */

/* fp16 1024.0. At this exponent the mantissa step is exactly 1, so OR-ing a
 * value in [0,15] into the low bits adds it exactly. 1032 = 1024 + 8 recentres
 * the levels in the same operation. */
